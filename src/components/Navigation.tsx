...
// Hoisted Car + Smoke Logo to avoid remounts on Navigation re-renders
const CarSmokeLogoComponent: React.FC<{ onClick: () => void }> = ({ onClick }) => {
	...
	const [play, setPlay] = useState(!hasPlayed);
	const [photoReady, setPhotoReady] = useState(false);

	useEffect(() => {
		if (!play) return; // Skip timer if we've already played
		const t = setTimeout(() => setPlay(false), 5200);
		return () => clearTimeout(t);
	}, [play]);

	// Load user-provided silhouette placed in public folder as /supercar-720s.png
	...
	return (
		<div className="relative flex items-center gap-3 sm:gap-4 select-none cursor-pointer" onClick={onClick}>
			{/* Road */}
			<motion.div
				className="relative h-10 sm:h-12 w-32 sm:w-48 rounded-2xl bg-[#000000] border border-transparent overflow-hidden"
				initial={false}
			...
			>
				...
				{photoReady ? (
					...
					<motion.div
						...
						onAnimationComplete={() => { sessionStorage.setItem('vlanco_nav_anim_played', '1'); setHasPlayed(true); setPlay(false); }}
					>
				...
				>
					...
					<path d="M24 80
						...
						C118 48, 88 52, 62 58
						C44 62, 32 68, 24 74 Z" fill="url(#carPaint)" filter="url(#glow)" />
				...
				{/* Smoke and effects should not render when play is false to avoid CPU work */}
				{play && (
					<>
						{[...Array(10)].map((_, i) => (
							<motion.div
						...
						{[...Array(5)].map((_, i) => (
							<motion.div
								...
								animate={{ opacity: [0, 1, 0], x: [0, -4 - i * 2], y: [0, -2 - i], scale: [0.5, 1.2, 0.6] }}
